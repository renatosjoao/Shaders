...
class Polyhedron{
  ...
  */  
  ...
  //*****************************************************************************************
  // READ MESH - MODIFICADO
  //*****************************************************************************************

      void readmesh(QVector4D *vertices, unsigned int n_vertices,unsigned int *indices, unsigned int n_faces){

        memoryallocate(n_vertices,n_faces);

        for(int i=0; i<n_vertices; i++){
          setPoint(i,vertices[i].x(),vertices[i].y(),vertices[i].z());
        }

        for(int i=0; i<n_faces; i++){
          setFace(i,indices[3*i],indices[3*i+1],indices[3*i+2]);
